        ...
        try:
            with open(cache_file, 'r') as f:
                ...
                if os.path.exists(session_dir):
                    ...
                    # Only use cache if we found enough frames
                    if len(frames) >= min(5, max_frames):
                        print(f"Using cached frames from {session_dir}")
                        return frames, session_dir
                    else:
                        print(f"Not enough valid frames found in cache ({len(frames)}), will extract new frames")
                else:
                    print(f"Cache session directory does not exist: {session_dir}")
        except Exception as e:
            print(f"Error reading cache: {str(e)}")
    else:
        print(f"No cache found for video with hash {video_hash}")
...
def save_to_frame_cache(video_path, frame_interval, max_frames, frames, session_dir):
    ...
    try:
        ...
        print(f"Saved {len(frames)} frames to cache file {cache_file}")
    except Exception as e:
...
def clean_old_caches(max_age_days=7):
    ...
    try:
        ...
        for cache_file in os.listdir(CACHE_DIR):
            ...
            if age_days > max_age_days:
                print(f"Removing old cache file: {cache_path} (age: {age_days} days)")
                os.remove(cache_path)
                count += 1
                
        return f"Cleaned {count} cache files older than {max_age_days} days"
    except Exception as e:
        print(f"Error cleaning old caches: {str(e)}")
        return f"Error cleaning old caches: {str(e)}"

# Create Gradio interface
with gr.Blocks() as demo:
    gr.Markdown("# Qwen2.5-VL Video Analysis with Time Frame Control and Improved Caching")
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    ...
                    clean_old_cache_btn = gr.Button("Clean Old Caches (>7 days)")
...
if __name__ == "__main__":
    ...
    print("Starting Gradio interface with time frame control and improved caching...")
